...
async function postForm<T>(url: string, data: URLSearchParams | FormData): Promise<T> {
  return fetchJson<T>(url, {
    method: 'POST',
    body: data,
  });
...
export function useUserInfo() {
  ...
  const login = useCallback(async (username: string, password: string): Promise<void> => {
    ...
    await postForm<ApiResponse<UserInfo>>('/login', formData);
    await fetchUserInfo();
  }, [fetchUserInfo]);
  ...
  const register = useCallback(async (username: string, password: string): Promise<void> => {
    ...
    formData.append('username', username);
    formData.append('password', password);
    await postForm<ApiResponse<UserInfo>>('/register', formData);
    await fetchUserInfo();
  }, [fetchUserInfo]);

  const logout = useCallback(async (): Promise<void> => {
    await fetchJson<ApiResponse<null>>('/logout');
    setUserInfo(null);
  }, []);

  return { userInfo, loading, error, login, register, logout, refetch: fetchUserInfo };
}
...
export function useOpenApp() {
  ...
  const openApp = useCallback(async (title: string, url: string): Promise<void> => {
    try {
      setOpening(true);
...
export function useMusicPlaylists() {
  ...
  const fetchPlaylists = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetchJson<ApiResponse<Playlist[]>>('/api/get_music_playlists');
      if (res.success && res.data) {
